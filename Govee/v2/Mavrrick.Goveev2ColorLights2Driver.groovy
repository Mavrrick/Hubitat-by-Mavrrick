...
metadata {
	definition(name: "Govee v2 Color Lights 2 Driver", namespace: "Mavrrick", author: "Mavrrick") {
		capability "Switch"
        capability "Actuator"
...
def setNextEffect() {
    ...
    if (device.currentValue("effectNum") == "0" || device.currentValue("effectNum") == device.getDataValue("maxScene")) {
        ...
        keys = []
        names = []
        keys.addAll(state.sceneOptions.keySet())
        names.addAll(state.sceneOptions.values())
