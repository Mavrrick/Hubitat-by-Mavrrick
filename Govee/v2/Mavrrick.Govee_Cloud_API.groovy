...
private def sendCommand(String command, payload2, type) {
    ...
    asynchttpPost('sendCmdResp',params)
...
def sendCmdResp(resp, data){
                if (debugLog) { log.debug "sendCommand(): Response data is "+resp.json }
                code = resp.json.code
                command = resp.json.capability.instance
                payload2 = resp.json.capability.value  
                if (debugLog) { log.debug "sendCommand(): Code = ${code} command = ${command} value = ${payload2}" }
                if (code == 200 && command == "powerSwitch") {
                    sendEvent(name: "cloudAPI", value: "Success")
                    if (payload2 == 1) {
                        sendEvent(name: "switch", value: "on")
                   ...
                   sendEvent(name: "cloudAPI", value: "Success")
                   sendEvent(name: "switch", value: "on")
                   sendEvent(name: "mode", value: payload2.workMode)
                   if (payload2.modeValue == 0) {
                       sendEvent(name: "modeValue", value: "N/A")
                   } else {
                       sendEvent(name: "modeValue", value: payload2.modeValue)
                   }
                   setModeDescription(payload2.workMode)
                   if (descLog) { log.info "${device.label} workMode was set to ${payload2}"}
                   }
               else if (code == 200 && command == "sliderTemperature") {
                   sendEvent(name: "cloudAPI", value: "Success")
                   def units = payload2?.unit.charAt(0)
                   sendEvent(name: "switch", value: "on")
                   sendEvent(name: "targetTemp", value: payload2?.temperature, unit: units)
                   sendEvent(name: "targetTempUnit", value: units)
                   if (descLog) { log.info "${device.label} TargetTemp was set to ${payload2?.temperature}°${units}"}
                   }
               else if (code == 200 && command == "targetTemperature") {
                   sendEvent(name: "cloudAPI", value: "Success")
                   def units = payload2.unit.charAt(0)
...
def getDeviceState(){
    ...
    asynchttpPost('getDevStateResp',params)
...
def getDevStateResp(resp, data) {
                if (debugLog) { log.debug "getDeviceState():"+resp.json}
                resp.json.payload.capabilities.each {
                    type = it.type
                    instance = it.instance
                    if (it.state.value || it.state.value == 0) {
                    if (debugLog) { log.debug "getDeviceState(): ${type} ${instance} ${it.state.value}" }
...
def retrieveScenes2(){
    ...
    asynchttpPost('retrieveScene2Resp',params)    
/*    
try {
...
def retrieveScene2Resp(resp, data) {
                if (debugLog) { log.debug "retrieveScene2Resp():"+resp.json.payload.capabilities.parameters.options}
//                state.scenes = resp.data.payload.capabilities.parameters.options.get(0)
                state.remove("sceneOptions")
                state.scenes = [:]
                resp.json.payload.capabilities.parameters.options.get(0).each {
                    state.scenes.put(it.value.id,it.name)  
                } 
                if (debugLog) { log.debug "retrieveScene2Resp(): dynamic scenes loaded"}
/*                resp.headers.each {
                    if (debugLog) { log.debug "getDeviceState(): ${it.name}: ${it.value}"}                    
                    name = it.name
                    value = it.value
...
//    state.scenes = [] as List
    ...
    asynchttpPost('retrieveDIYScene2Resp',params)     
...
def retrieveDIYScene2Resp(resp, data) {

                if (debugLog) { log.debug "retrieveDIYScenes():"+resp.json.payload.capabilities}
                state.remove("diyEffects")
                state.remove("diyScene")                
                state.diySceneOptions = [:]
                resp.json.payload.capabilities.parameters.options.get(0).each {
