// Hubitat driver for Govee H7123 Air Purifier
// Version 2.0.0
//
...
metadata {
    definition(name: "Govee v2 H7123 Air Purifier", namespace: "Mavrrick", author: "Mavrrick") {
        capability "Switch"
        capability "Actuator"
...
def autoMode() {
    if (debugLog) {log.debug "auto(): Processing Working Mode command 'Auto' "}
...
def sleepMode() {
    if (debugLog) {log.debug "sleep(): Processing Working Mode command 'Sleep' "}
    sendEvent(name: "cloudAPI", value: "Pending")
...
/* def setFanSpeed(gear) {
    if (debugLog) {log.debug "setFanSpeed(): Processing Working Mode command 'setFanSpeed' to ${gear} "}
    sendEvent(name: "cloudAPI", value: "Pending")
    switch(gear){
        case "Low":
            gear = 1;
